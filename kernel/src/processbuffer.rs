...
impl<'a> ProcessSliceBuffer<'a> {
    ...
    pub fn offset(&self) -> Result<usize, ErrorCode> {
        ...
        //
        // TODO: should remove the index operator and replace with a simple
        // .get(Self::RANGE_FLAGS.start) once #4192 is merged
        if (self.slice.get(Self::RANGE_FLAGS).ok_or(ErrorCode::SIZE)?)[0].get() != Self::FLAGS_V0 {
    ...
    fn set_offset(&self, offset: usize) -> Result<(), ErrorCode> {
        ...
        self.slice
            ...
            .ok_or(ErrorCode::SIZE)?
            .iter()
            .for_each(|flag_byte| flag_byte.set(0));
